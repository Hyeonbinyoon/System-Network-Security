...
uint16_t hb_ip_hdr::offset_value() const
{
    return ntohs(offset);
}
...
uint32_t hb_tcp_hdr::seq_num_value() const
{
    return ntohl(seq_num);
}

uint32_t hb_tcp_hdr::ack_num_value() const
{
    return ntohl(ack_num);
}

uint8_t hb_tcp_hdr::flags_value() const
{
    return flags;
}
